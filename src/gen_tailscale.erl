...
%%
-doc """
Start a loopback server using the libtailscale loopback function.

This function starts a loopback address server that provides both SOCKS5 proxy
functionality and LocalAPI access for the Tailscale network.

## Parameters

- `Options` - A list of options for configuring the loopback server:
  - `{hostname, string()}` - The hostname to use for the Tailscale node
  - `{ephemeral, boolean()}` - Whether to create an ephemeral node (default: false)

## Returns

- `{ok, {Address, ProxyCred, LocalApiCred}}` on success, where:
  - `Address` - String containing the host:port where the loopback server is listening
  - `ProxyCred` - 32-character string credential for SOCKS5 proxy authentication
  - `LocalApiCred` - 32-character string credential for LocalAPI access
- `{error, Reason}` on failure

## Usage

The loopback server serves two main purposes:

1. **SOCKS5 Proxy**: Can be used as a SOCKS5 proxy onto the tailnet
   - Authentication: username "tsnet", password `ProxyCred`

2. **LocalAPI Server**: Serves the Tailscale "LocalAPI" on `/localapi` endpoints
   - Requires both:
     - `Sec-Tailscale: localapi` HTTP header
     - Basic auth with `LocalApiCred` as password (empty username)

## Example

```erlang
{ok, {Addr, ProxyCred, LocalApiCred}} = 
    gen_tailscale:start_loopback([{hostname, "my-node"}, {ephemeral, true}]),
io:format("Loopback server at: ~s~n", [Addr]),
io:format("Proxy credential: ~s~n", [ProxyCred]),
io:format("LocalAPI credential: ~s~n", [LocalApiCred]).
```
""".
-spec start_loopback(Options) -> {ok, {Address, ProxyCred, LocalApiCred}} | {error, Reason} when
      Options :: [Option],
      Option :: {hostname, string()} | {ephemeral, boolean()},
      Address :: string(),
      ProxyCred :: string(),
      LocalApiCred :: string(),
      Reason :: term().
...
start_loopback(Options) ->
    gen_tailscale_socket:start_loopback(Options).
